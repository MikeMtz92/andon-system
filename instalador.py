...
from ReAction import (
    validar_licencia_online, iniciar_demo, guardar_config_licencia, 
    guardar_config_db, probar_conexion_mysql, DB_CONFIG_DEFAULT
)

# Importamos la función de creación de tablas MySQL
...
class InstaladorAndon:
    def __init__(self):
        ...
        self.root.after(100, self.iniciar_asistente)
        self.root.mainloop()
    
    def iniciar_asistente(self):
        """Inicia el flujo de instalación"""
        self.paso1_activacion_licencia()
    ...
    def validar_licencia_ingresada(self):
        ...
        valido, info = validar_licencia_online(license_key, self.installation_id)
    ...
    def iniciar_demo_desde_instalador(self):
        """Inicia el periodo de demo"""
        exito, info = iniciar_demo(self.installation_id)
    ...
    def paso2_seleccion_bd(self):
        ...
        tk.Button(self.root, text="▶ CONTINUAR", 
              command=self.paso2_5_personalizar_fallas,  # Cambiado
    ...
    def paso2_5_personalizar_fallas(self):
        ...
        self.mapeo_vars_instalacion = {}
        self.mapeo_labels = {}  # Para guardar referencias a los labels de "Sin tipo disponible"
    ...
    def _actualizar_comboboxes_botones(self):
        """Actualiza los valores de los comboboxes de asignación de botones"""
        try:
            # Obtener lista actualizada de tipos
            tipos_actualizados = []
            for tipo_data in self.tipos_instalacion:
            ...
            # Actualizar cada combobox de botones
            for num_boton, var in self.mapeo_vars_instalacion.items():
                # Buscar el combobox asociado a esta variable
                    ...
                    if isinstance(widget, tk.Frame):
                        for child in widget.winfo_children():
                            if isinstance(child, ttk.Combobox) and child.cget('textvariable') == str(var):
                                ...
                                child['values'] = tipos_actualizados
                                
                                # Si el valor actual ya no está en la lista, limpiarlo
                                if var.get() and var.get() not in tipos_actualizados:
                                    var.set("")
                                break
        except Exception as e:
            print(f"Error actualizando comboboxes: {e}")
    ...
    def probar_conexion_mysql(self):
        ...
        try:
            ...
            exito, msg = probar_conexion_mysql(config)
            if exito:
    ...
    def paso3_confirmar_instalacion(self):
        ...
        # Preparar configuración de BD (código existente)...
        try:
            self.config_db = {
                "tipo": "mysql",
                "host": self.mysql_vars["Host:"].get(),
    ...
    def ejecutar_instalacion(self):
        ...
        try:
            ...
            exito, msg = probar_conexion_mysql(self.config_db)
            if not exito:
                print(f"    ❌ Error: {msg}")
            ...
            # 2. Verificar/Crear base de datos (VERSIÓN MEJORADA)
            ...
            try:
                ...
                try:
                    conn = mysql.connector.connect(
                        ...
                        database=self.config_db["base_datos"],  # Intentar conectar directamente
                    ...
                    print("    ⏭️  Saltando creación de base de datos...")
                    
                except mysql.connector.Error as e:
                    # Si el error es que la BD no existe (1049), entonces intentamos crearla
                    if "1049" in str(e):
                ...
                if "1045" in str(e):
                    ...
                    print("\n💡 SOLUCIÓN: El usuario no tiene permisos para crear bases de datos")
            ...
            print("\n[3/7] Creando tablas...")
            print("    Llamando a crear_tablas_mysql()...")
            try:
            ...
            try:
                guardar_config_licencia(self.config_licencia, self.config_db)
            ...
            try:
                guardar_config_db(self.config_db)
            ...
            try:
                from ReAction import guardar_tipos_falla
                # USAR LOS TIPOS PERSONALIZADOS EN LUGAR DE LOS POR DEFECTO
                guardar_tipos_falla(self.tipos_falla_instalacion, self.config_db)
            ...
            try:
                from ReAction import guardar_config_sistema
                ...
                guardar_config_sistema(config_sistema_default, self.config_db)
            ...
            if hasattr(self, 'mapeo_instalacion') and self.mapeo_instalacion:
                ...
                try:
                    from ReAction import guardar_mapeo_botones
                    guardar_mapeo_botones(self.mapeo_instalacion, self.config_db)
    ...
    def iniciar_aplicacion(self):
        ...
        try:
            from ReAction import AndonApp, cargar_config_db, cargar_config_licencia, cargar_tipos_falla, cargar_config_sistema
